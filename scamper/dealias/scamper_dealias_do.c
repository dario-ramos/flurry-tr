                                            ...
                                            scamper_addr_t *addr)
{
  ...
  if ((tgt = dealias_target_find (state, addr)) != NULL)
    return tgt;
  if ((tgt = malloc_zero(sizeof(dealias_target_t))) == NULL || (tgt->probes =
      dlist_alloc ()) == NULL)
  {
...
dealias_probe_icmp_find(dealias_state_t *state, dealias_target_t *tgt,
                        uint16_t ipid, uint8_t type, uint8_t code, uint16_t id,
                        uint16_t seq)
{
  scamper_dealias_probedef_t *def;
  dealias_probe_t *dp;
  ...
  if ((SCAMPER_ADDR_TYPE_IS_IPV4(tgt->addr) && type == ICMP_ECHO && code == 0)
...
static void do_dealias_handle_dl(scamper_task_t *task, scamper_dl_rec_t *dl)
{
  ...
  else if (state->flags & DEALIAS_STATE_FLAG_DL && SCAMPER_DL_IS_ICMP(dl))
  {
    ...
    SCAMPER_DL_IS_ICMP_PACKET_TOO_BIG(dl))
    {
      ...
      if (scamper_dl_rec_icmp_ip_dst (dl, &a) != 0 || (tgt =
          dealias_target_find (state, &a)) == NULL)
        return;
...
err:
  if (reply != NULL)
    scamper_dealias_reply_free (reply);
  dealias_handleerror (task, errno);
  return;
}
                                   ...
                                   scamper_icmp_resp_t *ir)
{
  ...
  SCAMPER_ICMP_RESP_IS_PACKET_TOO_BIG(ir))
  {
    ...
    if (scamper_icmp_resp_inner_dst (ir, &a) != 0 || (tgt =
        dealias_target_find (state, &a)) == NULL)
      return;
  ...
  else if (SCAMPER_ICMP_RESP_IS_ECHO_REPLY(ir) != 0)
  {
    ...
    (tgt = dealias_target_find(state, &a)) == NULL)
      return;
    probe = dealias_probe_echoreq_find (state, tgt, ir->ir_icmp_id,
                                        ir->ir_icmp_seq);
  }
...
{
  static int (*const postprobe_func[])(scamper_dealias_t*, dealias_state_t*) =
  {
...
err:
  if (pd[0].dst != NULL)
    scamper_addr_free (pd[0].dst);
  if (pd[1].dst != NULL)
    scamper_addr_free (pd[1].dst);
  return -1;
}
                                             ...
                                             scamper_cycle_t *cycle)
{
  ...
  if ((state = malloc_zero(sizeof(dealias_state_t))) == NULL
      || (state->recent_probes = dlist_alloc ()) == NULL || (state->ptbq =
          slist_alloc ()) == NULL || (state->discard = slist_alloc ()) == NULL
      || (state->targets = splaytree_alloc (
          (splaytree_cmp_t) dealias_target_cmp)) == NULL)
  {
