...
static char* dealias_header_tostr (const scamper_dealias_t *dealias)
{
  static const char *pf_flags[] =
    { "nobs", "csa" };
  static const char *rg_flags[] =
    { "nobs" };
  static const char *ally_flags[] =
    { "nobs" };
  ...
  string_concat (buf, sizeof(buf), &off,
                 ", \"start\":{\"sec\":%u, \"usec\":%u}", dealias->start.tv_sec,
                 dealias->start.tv_usec);
...
static char* dealias_reply_tostr (const scamper_dealias_reply_t *reply)
{
  ...
  string_concat (
      ...
      scamper_addr_tostr (reply->src, tmp, sizeof(tmp)), reply->rx.tv_sec,
      reply->rx.tv_usec, reply->ttl);
