...
static void dealias_prefixscan_free (void *data)
{
  ...
  if (prefixscan->a != NULL)
    scamper_addr_free (prefixscan->a);
  if (prefixscan->b != NULL)
    scamper_addr_free (prefixscan->b);
  if (prefixscan->ab != NULL)
    scamper_addr_free (prefixscan->ab);
    ...
    const scamper_dealias_probedef_t *d, char *b, size_t l)
{
  static const char *m[] =
    {
    NULL, "icmp-echo", "tcp-ack", "udp", "tcp-ack-sport", "udp-dport",
        "tcp-syn-sport", };
                                    ...
                                    int fudge)
{
  ...
  for (x = 0; x < dealias->probec; x++)
  {
    ...
    if (array_insert ((void***) &dr[pid].probes, &dr[pid].probec, pr, NULL)
        != 0)
      goto err;
  }
...
void scamper_dealias_free (scamper_dealias_t *dealias)
{
  ...
  if (dealias->cycle != NULL)
    scamper_cycle_free (dealias->cycle);
  if (dealias->list != NULL)
