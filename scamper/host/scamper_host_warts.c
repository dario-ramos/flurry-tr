                                  ...
                                  warts_addrtable_t *table)
{
  ...
  assert(
      x == SCAMPER_HOST_RR_DATA_TYPE_ADDR || x == SCAMPER_HOST_RR_DATA_TYPE_STR || x == SCAMPER_HOST_RR_DATA_TYPE_SOA || x == SCAMPER_HOST_RR_DATA_TYPE_MX);
                                   ...
                                   const uint16_t params_len)
{
  ...
  if (warts_list_getid (sf, host->list, &list_id) == -1)
    return -1;
  if (warts_cycle_getid (sf, host->cycle, &cycle_id) == -1)
    return -1;
                                  ...
                                  const scamper_host_t *host)
{
  ...
  if (warts_host_params_write (host, sf, table, buf, &off, len, flags,
