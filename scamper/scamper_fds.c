...
static void fd_refcnt_0(scamper_fd_t *fdn)
{
   ...
   */
  if (fdn->type == SCAMPER_FD_TYPE_PRIVATE || fdn->type == SCAMPER_FD_TYPE_FILE)
  {
    fd_free (fdn);
    return;
  }
...
static void fds_poll_check(short event, int rc, int count)
{
  ...
  for (i = 0; i < count; i++)
  {
     ...
     * of it goes.
     */
    if (poll_fds[i].fd >= 0 && poll_fds[i].fd < fd_array_s && (fd =
        fd_array[poll_fds[i].fd]) != NULL)
    {
      if (event == POLLIN)
...
{
  assert(
      type == SCAMPER_FD_TYPE_TCP4 || type == SCAMPER_FD_TYPE_TCP6 || type == SCAMPER_FD_TYPE_UDP4 || type == SCAMPER_FD_TYPE_UDP6 || type == SCAMPER_FD_TYPE_UDP4DG || type == SCAMPER_FD_TYPE_ICMP4 || type == SCAMPER_FD_TYPE_ICMP6);
...
static int fd_cmp(const scamper_fd_t *a, const scamper_fd_t *b)
{
  ...
  switch (a->type)
  {
    case SCAMPER_FD_TYPE_TCP4:
    case SCAMPER_FD_TYPE_TCP6:
      if (a->fd_tcp_sport < b->fd_tcp_sport)
        return -1;
      if (a->fd_tcp_sport > b->fd_tcp_sport)
        return 1;
    ...
    case SCAMPER_FD_TYPE_UDP4DG:
    case SCAMPER_FD_TYPE_UDP6:
      if (a->fd_udp_sport < b->fd_udp_sport)
        return -1;
      if (a->fd_udp_sport > b->fd_udp_sport)
        return 1;
      return fd_addr_cmp (a->type, a->fd_udp_addr, b->fd_udp_addr);
    ...
    case SCAMPER_FD_TYPE_DL:
      if (a->fd_dl_ifindex < b->fd_dl_ifindex)
        return -1;
      if (a->fd_dl_ifindex > b->fd_dl_ifindex)
        return 1;
      return 0;
...
static scamper_fd_t* fd_null(int type)
{
  ...
  if (fd == -1 || (fdn = fd_alloc (type, fd)) == NULL || (fdn->fd_tree_node =
      splaytree_insert (fd_tree, fdn)) == NULL || (fdn->fd_list_node =
      dlist_tail_push (fd_list, fdn)) == NULL)
  {
    goto err;
  }
...
int scamper_fds_init()
{
  ...
  if ((fd_list = alloc_list ("fd_list")) == NULL
      || (read_fds = alloc_list ("read_fds")) == NULL || (read_queue =
          alloc_list ("read_queue")) == NULL
      || (write_fds = alloc_list ("write_fds")) == NULL || (write_queue =
          alloc_list ("write_queue")) == NULL
