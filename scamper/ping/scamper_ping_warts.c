                               ...
                               uint16_t *flags_len, uint16_t *params_len)
{
  ...
  *params_len = 0;
  ...
  for (i = 0; i < sizeof(ping_vars) / sizeof(warts_var_t); i++)
  {
    ...
    if (var->id == WARTS_PING_ADDR_SRC_GID || var->id == WARTS_PING_ADDR_DST_GID
                                    ...
                                    const uint16_t params_len)
{
  ...
  if (warts_list_getid (sf, ping->list, &list_id) == -1)
    return -1;
  if (warts_cycle_getid (sf, ping->cycle, &cycle_id) == -1)
    return -1;
                                   ...
                                   const scamper_ping_t *ping)
{
  ...
  if (warts_ping_params_write (ping, sf, table, buf, &off, len, flags,
                               flags_len, params_len) == -1)
  {
    goto err;
  }
