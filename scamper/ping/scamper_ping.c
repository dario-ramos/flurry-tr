...
void scamper_ping_free (scamper_ping_t *ping)
{
  ...
  if (ping->dst != NULL)
    scamper_addr_free (ping->dst);
  if (ping->src != NULL)
    scamper_addr_free (ping->src);
  ...
  if (ping->probe_tsps != NULL)
    scamper_ping_v4ts_free (ping->probe_tsps);
  if (ping->probe_data != NULL)
    free (ping->probe_data);
