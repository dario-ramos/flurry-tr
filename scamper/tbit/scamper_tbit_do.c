...
static ssize_t tbit_ssl_init (tbit_state_t *state, char *name)
{
  ...
  if ((state->wbio = BIO_new (BIO_s_mem ())) == NULL
      || (state->rbio = BIO_new (BIO_s_mem ())) == NULL || (state->ssl =
          SSL_new (ssl_ctx)) == NULL)
    return -1;
...
static int tbit_app_http_req (scamper_task_t *task)
{
  ...
  string_concat (buf, sizeof(buf), &off, "Connection: Keep-Alive\r\n"
                 "Accept: */*\r\n"
                 "User-Agent: %s\r\n\r\n",
                 http_ua);
...
static int tbit_app_rx (scamper_task_t *task, uint8_t *data, uint16_t len)
{
  ...
  assert(
      tbit->app_proto == SCAMPER_TBIT_APP_HTTP || tbit->app_proto == SCAMPER_TBIT_APP_BGP);
...
static int dl_data_blindrst (scamper_task_t *task, scamper_dl_rec_t *dl)
{
  ...
  assert(
      tbit->type == SCAMPER_TBIT_TYPE_BLIND_RST || tbit->type == SCAMPER_TBIT_TYPE_BLIND_SYN);
