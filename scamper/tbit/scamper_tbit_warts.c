                                                ...
                                                uint32_t len)
{
  ...
  if (warts_params_read (buf, off, len, handlers, handler_cnt) != 0 || (pkt =
      scamper_tbit_pkt_alloc (dir, data, plen, &tv)) == NULL)
    goto err;
                                    ...
                                    const uint16_t params_len)
{
  ...
  if (warts_list_getid (sf, tbit->list, &list_id) == -1)
    return -1;
  if (warts_cycle_getid (sf, tbit->cycle, &cycle_id) == -1)
    return -1;
                                   ...
                                   const scamper_tbit_t *tbit)
{
  ...
  if (warts_tbit_params_write (tbit, sf, table, buf, &off, len, flags,
