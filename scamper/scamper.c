...
static void usage(uint32_t opt_mask)
{
  ...
  fprintf (
      stderr, "usage: scamper [-?Dv] [-c command] [-p pps] [-w window]\n"
...
static int check_options(int argc, char *argv[])
{
  static const scamper_multicall_t multicall[] =
    {
      ...
      { "scamper-sting", "sting", scamper_do_sting_arg_validate,
          ...
          { "scamper-neighbourdisc", "neighbourdisc",
              scamper_do_neighbourdisc_arg_validate,
              scamper_do_neighbourdisc_usage },
          ...
          { "scamper-traceb", "traceb", scamper_do_traceb_arg_validate,
              scamper_do_traceb_usage }, };
  int i;
  ...
  if ((options & (OPT_CTRL_INET | OPT_CTRL_UNIX | OPT_IP | OPT_INFILE)) != 0
      && scamper_command_set (
          (options & OPT_COMMAND) ? opt_command : SCAMPER_COMMAND_DEF) != 0)
  {
    return -1;
  }
  ...
  if (options & (OPT_CTRL_INET | OPT_CTRL_REMOTE | OPT_CTRL_UNIX))
  {
    ...
    if (options & OPT_CTRL_INET)
    {
      ...
      if (arglist_len != 0
          || string_addrport (opt_ctrl_inet, &ctrl_inet_addr, &ctrl_inet_port)
              != 0)
      {
        usage (OPT_CTRL_INET);
        return -1;
    ...
    if (options & OPT_CTRL_REMOTE)
    {
       ...
       */
      if (arglist_len != 0
          || string_addrport (opt_ctrl_remote, &ctrl_rem_name, &ctrl_rem_port)
              != 0)
      {
...
static int scamper(int argc, char *argv[])
{
  ...
  if (scamper_do_trace_init () != 0 || scamper_do_traceb_init () != 0
      || scamper_do_ping_init () != 0 || scamper_do_tracelb_init () != 0
      || scamper_do_dealias_init () != 0 || scamper_do_sting_init () != 0
      || scamper_do_neighbourdisc_init () != 0 || scamper_do_tbit_init () != 0
      || scamper_do_sniff_init () != 0 || scamper_do_host_init () != 0)
  {
    return -1;
  }
  ...
  for (;;)
  {
    ...
    {
      /* write the data out */
      if ((source = scamper_task_getsource (task)) != NULL && (sofname =
          scamper_source_getoutfile (source)) != NULL && (sof =
          scamper_outfiles_get (sofname)) != NULL)
      {
