                                                 ...
                                                 uint32_t len)
{
  ...
  if (warts_params_read (buf, off, len, handlers, handler_cnt) != 0 || (pkt =
      scamper_sting_pkt_alloc (flags, data, plen, &tv)) == NULL)
    goto err;
                                    ...
                                    const uint16_t params_len)
{
  ...
  if (warts_list_getid (sf, sting->list, &list_id) == -1)
    return -1;
  if (warts_cycle_getid (sf, sting->cycle, &cycle_id) == -1)
    return -1;
