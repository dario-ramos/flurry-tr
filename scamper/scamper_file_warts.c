                 ...
                 scamper_addr_t **out, void *param)
{
  ...
  if (type == 0 || type > SCAMPER_ADDR_TYPE_MAX)
    return -1;
  if ((wa = malloc_zero(sizeof(warts_addr_t))) == NULL || (wa->addr =
      scamper_addr_alloc (type, &buf[*off])) == NULL
...
{
  if (cycle->cycle != NULL)
    scamper_cycle_free (cycle->cycle);
  free (cycle);
  return;
}
                      ...
                      const int type, uint32_t *id)
{
  ...
  warts_cycle_params_write (cycle, buf, &off, len, flags, flags_len,
                            params_len);
                            ...
                            uint16_t *type, void **data)
{
  static const warts_obj_read_t objread[] =
    {
    NULL, (warts_obj_read_t) warts_list_read,
  ...
  for (;;)
  {
    ...
    *data = NULL;
    ...
    if (hdr.type == SCAMPER_FILE_OBJ_ADDR || hdr.type == SCAMPER_FILE_OBJ_LIST
...
err:
  if (state != NULL)
  {
    ...
    if (state->list_table != NULL)
      free (state->list_table);
    if (state->cycle_table != NULL)
      free (state->cycle_table);
    free (state);
  }
  return -1;
}
...
int scamper_file_warts_init_write(scamper_file_t *sf)
{
  ...
  if ((s->list_tree = splaytree_alloc ((splaytree_cmp_t) warts_list_cmp))
      == NULL)
    goto err;
  s->list_count = 1;
...
err:
  ...
  {
    if (s->list_tree != NULL)
      splaytree_free (s->list_tree, NULL);
    if (s->cycle_tree != NULL)
      splaytree_free (s->cycle_tree, NULL);
    free (s);
  }
  return -1;
}
