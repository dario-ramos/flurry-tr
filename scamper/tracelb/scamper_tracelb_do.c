...
{
  if (th->dn != NULL)
    dlist_node_pop (state->ths, th->dn);
  if (th->hostdo != NULL)
    scamper_host_do_free (th->hostdo);
  free (th);
  return;
}
...
static int tracelb_process_hops(scamper_task_t *task, tracelb_branch_t *br)
{
  ...
  for (i = 0; i < br->newnodec; i++)
  {
     ...
     */
    if ((set = scamper_tracelb_probeset_alloc ()) == NULL || (flowids =
        slist_alloc ()) == NULL)
    {
      if (br->mode != MODE_CLUMP)
    ...
    if (br->mode == MODE_FIRSTHOP)
    {
      ...
      if ((newp = tracelb_path_alloc (state, 0)) == NULL || (tlbl =
          tracelb_link_alloc (state, link, newp)) == NULL
...
static void tracelb_process_probes(scamper_task_t *task, tracelb_branch_t *br)
{
  ...
  assert(
      br->mode == MODE_FIRSTHOP || br->mode == MODE_HOPPROBE || br->mode == MODE_PERPACKET || br->mode == MODE_CLUMP);
                                ...
                                scamper_tracelb_reply_t *reply)
{
  ...
  if (branch->n >= TRACELB_CONFIDENCE_MAX_N
      || (scamper_addr_cmp (reply->reply_from, trace->dst) == 0
          && branch->newnodec < 2) || branch->k >= k (state, branch->n))
  {
                                   ...
                                   scamper_icmp_resp_t *ir)
{
  ...
  else if (SCAMPER_TRACELB_TYPE_IS_ICMP(trace))
  {
    if (SCAMPER_ICMP_RESP_IS_ECHO_REPLY(ir) == 0)
    {
      ...
      if (ir->ir_inner_ip_proto != proto || ir->ir_inner_icmp_id != trace->sport
...
static void handletimeout_hopprobe(scamper_task_t *task, tracelb_branch_t *br)
{
  ...
  assert(
      br->mode == MODE_FIRSTHOP || br->mode == MODE_HOPPROBE || br->mode == MODE_CLUMP);
...
static void do_tracelb_probe(scamper_task_t *task)
{
  ...
  if ((tp = malloc_zero(sizeof(tracelb_probe_t))) == NULL || (tp->probe =
      scamper_tracelb_probe_alloc ()) == NULL)
  {
  ...
  assert(
      (tpl != NULL && tpl->probe->rxc == 0 && tpl->probe->attempt+1 < trace->attempts) || branch->mode == MODE_FIRSTADDR || branch->mode == MODE_FIRSTHOP || branch->mode == MODE_HOPPROBE || branch->mode == MODE_PERPACKET || branch->mode == MODE_CLUMP);
