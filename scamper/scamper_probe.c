...
static void probe_free(scamper_probe_t *pr)
{
  ...
  if (pr->pr_ip_dst != NULL)
    scamper_addr_free (pr->pr_ip_dst);
  if (pr->pr_ip_src != NULL)
    scamper_addr_free (pr->pr_ip_src);
  if (pr->pr_ipopts != NULL)
    free (pr->pr_ipopts);
  if (pr->pr_data != NULL)
    free (pr->pr_data);
  free (pr);
...
int scamper_probe_task(scamper_probe_t *pr, scamper_task_t *task)
{
  ...
  if (SCAMPER_ADDR_TYPE_IS_IPV4(pr->pr_ip_dst))
  {
    if ((pr->pr_flags & SCAMPER_PROBE_FLAG_SPOOF) == 0 && (icmp =
        scamper_task_fd_icmp4 (task, pr->pr_ip_src->addr)) == NULL)
    {
      pr->pr_errno = errno;
      goto err;
    }
  ...
  {
    if ((pr->pr_flags & SCAMPER_PROBE_FLAG_SPOOF) == 0 && (icmp =
        scamper_task_fd_icmp6 (task, pr->pr_ip_src->addr)) == NULL)
    {
      pr->pr_errno = errno;
      goto err;
    }
