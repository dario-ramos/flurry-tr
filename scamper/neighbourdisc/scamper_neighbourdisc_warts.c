                                          ...
                                          uint32_t len)
{
  ...
  for (i = 0; i < pr->rxc; i++)
  {
    ...
    if (warts_neighbourdisc_reply_read (reply, state, table, buf, off, len)
        != 0)
      return -1;
  }
                                            ...
                                            const uint16_t params_len)
{
  ...
  if (warts_list_getid (sf, nd->list, &list_id) == -1)
    return -1;
  if (warts_cycle_getid (sf, nd->cycle, &cycle_id) == -1)
    return -1;
                                           ...
                                           const scamper_neighbourdisc_t *nd)
{
  ...
  if (nd->probec > 0)
  {
    ...
    for (i = 0; i < nd->probec; i++)
    {
      probe = nd->probes[i];
      len2 = len;
      if (warts_neighbourdisc_probe_state (sf, probe, &probes[i], table, &len2)
          != 0)
        goto err;
      if (len2 < len)
        goto err;
