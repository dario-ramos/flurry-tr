...
void scamper_task_sig_free(scamper_task_sig_t *sig)
{
  ...
  switch (sig->sig_type)
  {
    case SCAMPER_TASK_SIG_TYPE_TX_IP:
      if (sig->sig_tx_ip_dst != NULL)
        scamper_addr_free (sig->sig_tx_ip_dst);
      if (sig->sig_tx_ip_src != NULL)
        scamper_addr_free (sig->sig_tx_ip_src);
      break;

    case SCAMPER_TASK_SIG_TYPE_TX_ND:
      if (sig->sig_tx_nd_ip != NULL)
        scamper_addr_free (sig->sig_tx_nd_ip);
      break;

    case SCAMPER_TASK_SIG_TYPE_SNIFF:
      if (sig->sig_sniff_src != NULL)
        scamper_addr_free (sig->sig_sniff_src);
      break;

    case SCAMPER_TASK_SIG_TYPE_HOST:
      if (sig->sig_host_name != NULL)
        free (sig->sig_host_name);
      break;
  }
...
static scamper_fd_t* task_fd(scamper_task_t *t, scamper_fd_t *fd)
{
  ...
  if (array_find ((void**) t->fds, t->fdc, fd,
                  (array_cmp_t) task_fd_cmp) == NULL)
  {
    if (array_insert ((void***) &t->fds, &t->fdc, fd, (array_cmp_t) task_fd_cmp)
        != 0)
    {
      scamper_fd_free (fd);
      return NULL;
