                                           ...
                                           scamper_icmp_resp_t *ir)
{
  ...
  if (ir->ir_ext != NULL
      && scamper_icmpext_parse (&hop->hop_icmpext, ir->ir_ext, ir->ir_extlen)
          != 0)
  {
    goto err;
  }
                            ...
                            trace_probe_t *probe)
{
  ...
  assert(
      state->mode == MODE_TRACE || state->mode == MODE_DTREE_FWD || state->mode == MODE_DTREE_BACK);
...
static void do_trace_handle_dl(scamper_task_t *task, scamper_dl_rec_t *dl)
{
    ...
    else if (SCAMPER_DL_IS_ICMP(dl))
    {
      ...
      else
      {
        ...
        if (dl->dl_af == AF_INET)
        {
          ...
          else if (trace_ipid_fudge (state, dl->dl_icmp_ip_id, &probe_id) != 0)
          {
            return;
          }
        }
      ...
      || trace->type == SCAMPER_TRACE_TYPE_ICMP_ECHO_PARIS)
  {
    ...
    else if ((SCAMPER_DL_IS_ICMP_TTL_EXP(dl) || SCAMPER_DL_IS_ICMP_UNREACH(dl)
...
static void trace_pmtud_state_free(trace_pmtud_state_t *state)
{
  ...
  if (state->TTL != NULL)
    free (state->TTL);
  if (state->note != NULL)
    scamper_trace_pmtud_n_free (state->note);
  free (state);
  return;
}
...
static void trace_state_free(trace_state_t *state)
{
  ...
  if (state->lss != NULL)
    free (state->lss);
  if (state->pmtud != NULL)
    trace_pmtud_state_free (state->pmtud);
...
static void do_trace_probe(scamper_task_t *task)
{
  ...
  assert(
      trace->dst->type == SCAMPER_ADDR_TYPE_IPV4 || trace->dst->type == SCAMPER_ADDR_TYPE_IPV6);
  ...
  if (state->dl != NULL
      && (state->mode == MODE_PMTUD_DEFAULT
          || state->mode == MODE_PMTUD_SILENT_L2
          || state->mode == MODE_PMTUD_SILENT_TTL
          || state->mode == MODE_PMTUD_BADSUGG || trace->offset != 0
          || trace->rtr != NULL
          || (SCAMPER_TRACE_TYPE_IS_UDP_PARIS(trace) && sunos != 0)
...
void* scamper_do_trace_alloc(char *str)
{
  ...
  if (gss != NULL)
  {
    if ((gss_tree = splaytree_alloc ((splaytree_cmp_t) scamper_addr_cmp))
        == NULL)
      goto err;
