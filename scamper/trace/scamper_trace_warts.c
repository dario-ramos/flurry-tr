...
static int warts_trace_params_read (scamper_trace_t *trace,
                                    warts_state_t *state,
                                     ...
                                     const uint16_t params_len)
{
  ...
  if (warts_list_getid (sf, trace->list, &list_id) == -1)
    return -1;
  if (warts_cycle_getid (sf, trace->cycle, &cycle_id) == -1)
    return -1;
                                   ...
                                   uint32_t *off, uint32_t len)
{
  ...
  {
    if (lss_stop != NULL)
      scamper_addr_free (lss_stop);
    if (gss_stop != NULL)
      scamper_addr_free (gss_stop);
    if (lss != NULL)
      free (lss);
    return -1;
  }
